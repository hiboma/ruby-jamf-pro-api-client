...
module JamfPro
  class VolumePurchasingLocationListView
    attr_accessor :name
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `JamfPro::VolumePurchasingLocationListView` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `JamfPro::VolumePurchasingLocationListView`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [name, total_purchased_licenses, total_used_licenses, id, apple_id, organization_name, token_expiration, country_code, location_name, client_context_mismatch, automatically_populate_purchased_content, send_notification_when_no_longer_assigned, auto_register_managed_users, site_id].hash
    end
