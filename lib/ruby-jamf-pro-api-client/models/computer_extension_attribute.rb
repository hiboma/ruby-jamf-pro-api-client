...
module JamfPro
  class ComputerExtensionAttribute
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `JamfPro::ComputerExtensionAttribute` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `JamfPro::ComputerExtensionAttribute`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      data_type_validator = EnumAttributeValidator.new('String', ["STRING", "INTEGER", "DATE_TIME"])
      return false unless data_type_validator.valid?(@data_type)
      input_type_validator = EnumAttributeValidator.new('String', ["TEXT", "POPUP", "SCRIPT", "LDAP"])
    ...
    def data_type=(data_type)
      validator = EnumAttributeValidator.new('String', ["STRING", "INTEGER", "DATE_TIME"])
    ...
    def input_type=(input_type)
      validator = EnumAttributeValidator.new('String', ["TEXT", "POPUP", "SCRIPT", "LDAP"])
    ...
    def hash
      [definition_id, name, description, enabled, multi_value, values, data_type, options, input_type].hash
    end
