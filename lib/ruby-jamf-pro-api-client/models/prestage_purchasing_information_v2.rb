...
module JamfPro
  class PrestagePurchasingInformationV2
    attr_accessor :id
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `JamfPro::PrestagePurchasingInformationV2` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `JamfPro::PrestagePurchasingInformationV2`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @leased.nil?
        invalid_properties.push('invalid value for "leased", leased cannot be nil.')
      end

      if @purchased.nil?
        invalid_properties.push('invalid value for "purchased", purchased cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @leased.nil?
      return false if @purchased.nil?
    ...
    def hash
      [id, leased, purchased, apple_care_id, po_number, vendor, purchase_price, life_expectancy, purchasing_account, purchasing_contact, lease_date, po_date, warranty_date, version_lock].hash
    end
