  ...
  class CloudLdapServerResponse
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `JamfPro::CloudLdapServerResponse` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `JamfPro::CloudLdapServerResponse`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      return false if !@connection_timeout.nil? && @connection_timeout < 5
      return false if !@search_timeout.nil? && @search_timeout < 5
    ...
    def hash
      [id, enabled, server_url, domain_name, port, keystore, connection_timeout, search_timeout, use_wildcards, connection_type].hash
    end
